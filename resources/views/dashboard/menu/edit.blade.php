...
<div class="p-6">
    <div class="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">Edit Menu</h2>
        ...
        <form action="{{ route('dashboard.menu.update', $menu->id_menu) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi_menu" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2E4239]"
                              placeholder="Masukkan deskripsi menu" required>{{ old('deskripsi_menu', $menu->deskripsi_menu) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="kategori" 
                        ...
                        <option value="Food" {{ old('kategori', $menu->kategori) == 'Food' ? 'selected' : '' }}>Makanan</option>
                        <option value="Drink" {{ old('kategori', $menu->kategori) == 'Drink' ? 'selected' : '' }}>Minuman</option>
                        <option value="Snack" {{ old('kategori', $menu->kategori) == 'Snack' ? 'selected' : '' }}>Snack</option>
                    </select>
                </div>

                <!-- Gambar Menu Baru (Optional) -->
                <div class="md:col-span-2">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Gambar Menu Baru (Opsional)</label>
